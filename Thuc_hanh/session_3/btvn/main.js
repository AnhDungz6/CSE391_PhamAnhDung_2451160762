...
function kiemTraHoTen(val) {
  ...
  const regex =
    /^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂÂÊÔƠƯưăâêôơưẠ-ỹ\s]+$/;
...
function kiemTraDiemTB(val) {
  ...
  if (score < 0 || score > 10)
    return "Điểm phải nằm trong khoảng từ 0.0 đến 10.0.";
  return "";
}
...
inputHoTen.addEventListener("input", () => {
  displayFieldError(
    inputHoTen,
    errHoTen,
    kiemTraHoTen(inputHoTen.value.trim()),
  );
});
...
inputNgaySinh.addEventListener("input", () => {
  displayFieldError(
    inputNgaySinh,
    errNgaySinh,
    kiemTraNgaySinh(inputNgaySinh.value),
  );
});
...
inputDiemTB.addEventListener("input", () => {
  displayFieldError(
    inputDiemTB,
    errDiemTB,
    kiemTraDiemTB(inputDiemTB.value.trim()),
  );
});
...
inputEmail.addEventListener("input", () => {
  displayFieldError(
    inputEmail,
    errEmail,
    kiemTraEmail(inputEmail.value.trim()),
  );
});
...
inputMatKhau.addEventListener("input", () => {
  displayFieldError(
    inputMatKhau,
    errMatKhau,
    kiemTraMatKhau(inputMatKhau.value.trim()),
  );
  ...
  if (inputConfirmMatKhau.value) {
    displayFieldError(
      inputConfirmMatKhau,
      errConfirmMatKhau,
      kiemTraConfirmMatKhau(
        inputConfirmMatKhau.value.trim(),
        inputMatKhau.value.trim(),
      ),
    );
  }
});
...
form.addEventListener("submit", (e) => {
  ...
  const errorConfirmMatKhauValue = kiemTraConfirmMatKhau(
    confirmMatKhau,
    matKhau,
  );
...
btnConfirmYes.addEventListener("click", () => {
  ...
  const targetName = studentToDelete
    ? studentToDelete.hoTen
    : pendingDeleteMaSV;
